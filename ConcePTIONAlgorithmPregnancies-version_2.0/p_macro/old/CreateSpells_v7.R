...
CreateSpells<-function(dataset,id,start_date,end_date,category,category_is_numeric=F,replace_missing_end_date,overlap=F,dataset_overlap){
  ...
  dataset %<>%
    ...
    dplyr::mutate(lag_end_date = if_else( row_number()>=3,lag(.data[[end_date]]),as.Date(ymd(19000101)))) %>% 
    dplyr::mutate(lag_max_end_date = if_else( row_number()>=3,lag(max_end_date_previous),as.Date(ymd(19000101)))) %>%
    dplyr::mutate(max_end_date_previous = if_else( row_number()>=3,max(lag_end_date,lag_max_end_date),max_end_date_previous))  %>%
