...
CountPersonTime<-function(Dataset_events, Dataset, Person_id, Start_study_time, End_study_time, Start_date, End_date, Birth_date = NULL, Strata = NULL,Outcomes, Name_event, Date_event, Age_bands = NULL, Unit_of_age = "year" , Increment = "year", include_remaning_ages = T, Aggregate = T){
    ...
    Dataset[,(date_cols) := lapply(.SD,function(x) (as.character(x))),.SDcols=date_cols]
    Dataset[,(date_cols) := lapply(.SD,function(x) (as.Date(x,"%Y%m%d"))),.SDcols=date_cols]
    Dataset_events[,eval(Date_event) := as.character(get(Date_event))]
    Dataset_events[,eval(Date_event):= as.Date(get(Date_event),"%Y%m%d")]
    ...
    test_overlap<-Dataset[!is.na(get(End_date))&!is.na(get(End_date))&get(End_date)>get(Start_date),][, eval(Person_id) := do.call(paste0,.SD), .SDcols=c(Person_id,Strata)][,.(get(Person_id), as.integer(get(Start_date)), as.integer(get(End_date)))]
    ...
    start<-as.Date(min(Dataset[,get(Start_date)]))
    end<-as.Date(max(Dataset[,get(End_date)]))
    ...
    Table_Temp <- list()
      for(i in as.list(Time_Increment)){
      
        first_day <- i
        last_day <- seq.Date(i, by=Increment, length=2)-1
        last_day <- last_day[2]
        
        Temp <- copy(Dataset)
        Temp[,eval(Increment) := as.Date(i)]
      ...
      if(!is.null(Age_bands)){
            ...
            if(Increment != "day" ){for(j in Age_bands){
              ...
              #Dataset[,age_start := CalAge(get(Start_date), get(Birth_date), Unit_of_age)]
              #Dataset[,age_end := CalAge(get(End_date), get(Birth_date), Unit_of_age)]
            }}
    ...
    if (Aggregate == T) {
      if (!is.null(Age_bands)) Dataset <- Dataset[, lapply(.SD, sum), .SDcols=paste0("Persontime_",Outcomes), by  = c(Person_id, Strata, Increment, "Ageband")]
      if (is.null(Age_bands)) Dataset <- Dataset[, lapply(.SD, sum), .SDcols=paste0("Persontime_",Outcomes), by  = c(Person_id, Strata, Increment)]
