...
groups_of_pregnancies_green<-groups_of_pregnancies_green[Episode==1,`:=`(group_start_date=min(group_start_date, pregnancy_start_date),group_end_date=min(group_end_date, pregnancy_end_date)), by=.(Episode, person_id)]
...
# reconciling with yellow, blue and red
...
groups_of_pregnancies_green_ybr1<-rbind(groups_of_pregnancies_green,groups_of_pregnancies_ybr_Ingreen,fill=TRUE)



#### continue with record in ybr that doesn't match in green
Ingreen<-groups_of_pregnancies_ybr_Ingreen[,pregnancy_id]
groups_of_pregnancies_ybr_NOTingreen<-groups_of_pregnancies_ybr[pregnancy_id%notin%Ingreen,]
...
groups_of_pregnancies_yellow_NOTingreen<-as.data.table(groups_of_pregnancies_yellow_NOTingreen %>% group_by(person_id) %>%  mutate (Episode=Reduce(sum, diff, accumulate = TRUE)+1))
