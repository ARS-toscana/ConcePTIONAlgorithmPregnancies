...
for (studyvar in study_variables_of_our_study){
  assign(studyvar, 
         get(load(paste0(dirtemp, studyvar, ".RData")))[person_id %in% sample_id, .(pregnancy_id = NA,
                                                                                    person_id,
                                                                                    n = as.integer(2),
...
concept_set_list_2 <- c("INSULIN", "gestational_diabetes", "fetal_nuchal_translucency", "amniocentesis", "Chorionic_Villus_Sampling", "others")
...
### final 
sample_from_pregnancies <- rbind(validation_sample, rbindlist(list_of_records))
sample_from_pregnancies <- sample_from_pregnancies[order(person_id, n)]
...
sample_from_pregnancies <- sample_from_pregnancies[n==1 | (record_date > pregnancy_start_date - 30 & record_date < pregnancy_end_date + 30)]
...
sample_from_pregnancies <- sample_from_pregnancies[, -c("person_id", "from_algorithm", "n")]
sample_from_pregnancies <- sample_from_pregnancies[record_date == as.Date("9999-12-31"), record_date := NA]

fwrite(sample_from_pregnancies, paste0(dirvalidation, "/sample_from_pregnancies.csv"))
