...
CreateSpells<-function(dataset,id,start_date,end_date,category,category_is_numeric=F,replace_missing_end_date,overlap=F,dataset_overlap){
  ...
  if(!missing(category)) {
    dataset<-dataset[, `:=`(num_spell = fifelse(rowid(get(id)) == 1, 1, 0)), by = list(get(id), get(category))][, `:=`(max_end_date_previous = fifelse(num_spell == 1, get(end_date), as.Date(ymd(19000101))))][, `:=`(max_end_date_previous = fifelse(rowid(get(id)) == 2, lag(get(end_date)), max_end_date_previous)), by = list(get(id), get(category))][, `:=`(lag_end_date = fifelse(rowid(get(id)) >= 3, lag(get(end_date)), as.Date(ymd(19000101)))), by = list(get(id), get(category))][, `:=`(lag_max_end_date = fifelse(rowid(get(id)) >= 3, lag(max_end_date_previous), as.Date(ymd(19000101)))), by = list(get(id), get(category))][, `:=`(max_end_date_previous = fifelse(rowid(get(id)) >=  3, max(lag_end_date, lag_max_end_date), max_end_date_previous)), by = list(get(id), get(category))][,`:=`(num_spell = fifelse(rowid(get(id)) > 1 & get(start_date) <= max_end_date_previous + 1, 0, 1)), by = list(get(id), get(category))][, `:=`(num_spell = cumsum(num_spell)), by = list(get(id), get(category))]
  } else {
    dataset<-dataset[, `:=`(num_spell = fifelse(rowid(get(id)) == 1, 1, 0))][, `:=`(max_end_date_previous = fifelse(num_spell == 1, get(end_date), as.Date(ymd(19000101))))][, `:=`(max_end_date_previous = fifelse(rowid(get(id)) == 2, lag(get(end_date)), max_end_date_previous))][, `:=`(lag_end_date = fifelse(rowid(get(id)) >= 3, lag(get(end_date)), as.Date(ymd(19000101))))][, `:=`(lag_max_end_date = fifelse(rowid(get(id)) >= 3, lag(max_end_date_previous), as.Date(ymd(19000101))))][, `:=`(max_end_date_previous = fifelse(rowid(get(id)) >=  3, max(lag_end_date, lag_max_end_date), max_end_date_previous))][,`:=`(num_spell = fifelse(rowid(get(id)) > 1 & get(start_date) <= max_end_date_previous + 1, 0, 1))][, `:=`(num_spell = cumsum(num_spell)), by = id]
  }
