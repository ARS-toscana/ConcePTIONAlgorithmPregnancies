...
ApplyComponentStrategy <- function(dataset,
    ...
    ####################################################################
    #check if there is missing in components
    vet<-NULL
    for (i in 0+1: dim(dataset)[1]){
      for(w in 0+1:numcomponents){
        if (is.na(dataset[i,eval(parse(text=components[w]))])) {
          print(paste(components[w],":","row",i,"is missing, so I delete it")) #print row with missing
          vet<-c(vet,i)
        }
      }
    }
    #delete row with missing
    if (is.null(vet)==FALSE) {    # if there is missing..
       dataset<-dataset[-as.numeric(levels(factor(vet))),] 
    }
    ###############################################################
    #check if there is no binary in components
    v<-NULL
    for (i in 0+1: dim(dataset)[1]){
      for(w in 0+1:numcomponents){
        if ( dataset[i,eval(parse(text=components[[w]]))] !=0 &  dataset[i,eval(parse(text=components[[w]]))] !=1 ) {
          print(paste(components[w],":","row",i,"is not binary, so I delete it")) #print row with missing
          v<-c(v,i)
        }
      }
    }
    #delete row with no binary
    if (is.null(v)==FALSE) {    # if there is missing..
      dataset<-dataset[-as.numeric(levels(factor(v))),] 
    }
    ######################################################################
    ...
    if (intermediate_output==T){
          ...
          fwrite(dataset,paste0(dirtemp,"/",intermediate_output_name,".csv") )
    ...
    if(!is.null(strata)==T){ p = p + facet_wrap(as.vector(strata))  }  # variabiles strata    
    if(!is.null(expected_number)==T){ p = p + geom_hline(aes(yintercept=PROP_TRUE,color="Reference"),size=1)}  #line PROP_TRUE

    #save graph in pdf
    ggsave(filename=paste0(diroutput,"/",figure_name," ",format(Sys.time(), "%Y_%m_%d-%H_%M"),".pdf"), plot=p,width=13.66,height=7.05,limitsize = FALSE)
    ### format(Sys.time(), "%Y_%m_%d-%H_%M") -> it is needed because it does not overwrite
    print(paste("Graph saved in",diroutput))
    }
    ...
    fwrite(x,paste0(diroutput,"/output.csv") )
