...
CountPersonTime<-function(Dataset_events, Dataset, Person_id, Start_study_time, End_study_time, Start_date, End_date, Birth_date = NULL, Strata = NULL,Outcomes, Name_event, Date_event, Age_bands = NULL, Unit_of_age = "year" , Increment = "year", include_remaning_ages = T, Aggregate = T){
    ...
    if (nrow(Recurrent) > 0){Recurrent <- Recurrent[get(Name_event) %in% Outcomes,]}
    ...
    colls_r <- Outcomes[Outcomes %in% colnames(Recurrent)] 
    
    Recurrent <- Recurrent[,paste0("zero_",eval(colls_r)) :=  lapply(.SD,is.na),.SDcols=eval(colls_r)]
    cols_s <- c("Person_id", paste0("zero_",eval(colls_r)))
    ...
    if(nrow(Recurrent2) > 0){Recurrent2 <- Recurrent2[get(Name_event) %in% Outcomes,]}
    ...
    colls_r2 <- Outcomes[Outcomes %in% colnames(Recurrent2)]
    
    lapply(colls_r2, function(x)  First_date[get(x) < get(Start_date), eval(paste0("zero2_",x)) :=  T])
    
    
    cols_s <- c(Person_id, paste0("zero2_",eval(colls_r2)))
    ...
    setnames(Dataset, "Person_id", eval(Person_id))
      ...
      #lapply(Outcomes, function(x) Dataset <- Dataset[, eval(x) := NA])
    ...
    lapply(colls_r, function(x)  Dataset[get(paste0("zero_",x)) == F, paste0("Persontime_",eval(x)) := 0  ])
    
    lapply(colls_r, function(x)  Dataset[,eval(paste0("zero_",x)) :=  NULL])
    }
    ...
    if(nrow(Recurrent2) > 0){
      ...
      #setnames(Dataset, "Person_id", eval(Person_id))
      lapply(colls_r2, function(x)  Dataset[get(paste0("zero2_",x)) == T, paste0("Persontime_",eval(x)) := 0  ])
      lapply(colls_r2, function(x)  Dataset[,eval(paste0("zero2_",x)) :=  NULL])
    }
