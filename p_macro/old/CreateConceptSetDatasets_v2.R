...
#'
CreateConceptSetDatasets <- function(dataset,codvar,datevar,dateformat, rename_col,
  ...
  alldomain <- unique(names(dataset))
  ...
  for (dom in alldomain) {
    ...
    for (df2 in dataset[[dom]]) {
      ...
      for (col in names(used_df)) { 
        for (i in 1:length(datevar[[df2]])) { 
          if (col == datevar[[df2]][[i]]) { 
          used_df<-used_df[, eval(col) :=as.Date(as.character(get(col)), dateformat)]
          }
        }
      }
      ...
      if(!missing(rename_col)){
        ...
        for (elem in names(rename_col)) {
          ...
          for (col in names(used_df)) {
            ...
            print(df2)
            print(col)
            #for (i in 1:length(data[[dom]][[df2]])) {
              if (col == data[[dom]][[df2]]) {
                print(paste0(col," è uguale a ",data[[dom]][[df2]]))
      ...
      for (concept in concept_set_dom[[dom]]) {
        if (concept %in% concept_set_names) {
          ...
          for (col in codvar[[concept_set_domains[[concept]]]][[df2]]) {
              ...
              for (single_cod in concept_set_codes[[concept]][[type_cod]]) {
                if (single_cod == "ALL_CODES") {
                  ...
                  if ((!missing(vocabulary))) {
                    if (dom %in% names(vocabulary)) {################### IF I GIVE VOCABULARY2 IN INPUT
                      used_df_typecod<-used_df[get(vocabulary[[dom]][df2]) == type_cod,]
                      used_df_typecod[(str_detect(gsub("\\.","",get(col)),gsub("\\.","",paste0("^",single_cod)))),c("Filter",paste0("Col_",concept)):= list(1,col)]
                      used_df0<-rbind(used_df0,used_df_typecod,fill=T)
                      }else{ 
            ...
            if (nrow(used_df0)!=0) used_df<-used_df0
                ...
                for (single_cod2 in concept_set_codes_excl[[concept]][[type_cod_2]]) {
                  if ((!missing(vocabulary))) {
                    if (dom %in% names(vocabulary)) {
                      print("vocabulary")
                      used_df<-used_df[get(vocabulary[[dom]][df2]) == type_cod_2,]
