...
Within each quality color, records of the same pregnancy can be concordant (equal start and end dates), slightly discordant (start and end date different by no more than 7 days) or discordant (start and end date different more than 7 days). The sampling method chosen for the verification process is stratified sampling. 
The strata were initially defined according to the color quality, and then,  within each color quality, divided by concordance. The following mutually exclusive strata were obtained:
...
The next graph describe the dimension of the sampling strata in percentage. 
...
plt <- ggplotly(plt)
plt  
sample_validated[sample == "Yellow_SlightlyDiscordant", sample := "Yellow_CP_SlightlyDiscordant"]
tmp <- merge(DT_recon, sample_validated[, .(N_verified = .N), sample] , by.x =  "strata", by.y = "sample")
tmp <- tmp[, Sampling_fraction_per_1000 := round(N_verified/N * 1000, 4)]
datatable(tmp[,.(Strata = strata, N_strata = N, Total = tot, Strata_percentage = percentage, N_verified, Sampling_fraction_per_1000)])
...
During the verification process, DAP were asked to provide judgements on the start, end, type and composition of pregnancies. 
For each of those variable a section is present showing the absolute results of the verification, the re-proportioning on the whole population and the re-proportioning on the strata defined by the color quality

### Start of Pregnancies

The following graph shows the results of the verification for the variable start of pregnancy.

```{r start}
DT_start <- copy(sample_validated)
#DT_start[sample == "Yellow_SlightlyDiscordant", sample := "Yellow_CP_SlightlyDiscordant"]
DT_start <- DT_start[, pregnancy_start_date_difference := as.character(pregnancy_start_date_difference)]


DT_start <- DT_start[tolower(pregnancy_start_date_correct) == "y", pregnancy_start_date_difference :="1. 0 days"]
...
DT_start <- DT_start[tolower(pregnancy_start_date_correct) == "unk_assessable", pregnancy_start_date_difference := "6. UNK ASSESSABLE"]
DT_start <- DT_start[tolower(pregnancy_start_date_correct) == "unk_not_assessable", pregnancy_start_date_difference := "7. UNK NOT ASSESSABLE"]
DT_start <- DT_start[tolower(pregnancy_start_date_correct) == "unk_appraisable", pregnancy_start_date_difference := "8. UNK APPRAISABLE"]
DT_start <- DT_start[pregnancy_start_date_difference == "" | is.na(pregnancy_start_date_difference), pregnancy_start_date_difference :="9. NOT SPECIFIED"]

DT_start[, color := str_split(sample, "_")[[1]][1], sample]

DT_start_agg <- DT_start[, .N, by = c("pregnancy_start_date_difference", "sample")]
DT_start_agg_plt <- merge(DT_start_agg, tmp[, .(strata, N_verified )] , by.x =  "sample", by.y = "strata")


plt <- ggplot(DT_start_agg_plt, aes(x = sample, y =N, fill = pregnancy_start_date_difference)) +
  geom_col()+
 scale_fill_manual(values = c("1. 0 days" = "slategray1", 
                               ...
                               "5. days > 90"="slategrey",
                               "6. UNK ASSESSABLE"= "orangered",
                               "7. UNK NOT ASSESSABLE"= "saddlebrown",
                               "8. UNK APPRAISABLE"= "yellow4",
                               "9. NOT SPECIFIED" = "tan2")) +
  scale_x_discrete(guide = guide_axis(angle = 45)) +
  guides(fill = guide_legend(title = "Verification: start difference")) +
  xlab("Strata")+
  ylab("N of pregnancies verified")+
  theme_bw()+
  theme(text = element_text(family = "serif"))

#plt <- ggplotly(plt,   xaxis = list(tickangle=45))
plt  
```
...
#### Reproportioning over the entire cohort 

The following graph shows the expected percentage of pregnancies in the total cohor with a different start date from that generated by the algorithm. 
The percentages are calculated taking into account the verification results and the sampling fraction.

$$ StartDifference_{C} = \sum_S \frac{N_{C, S}}{N_{S}} \times StrataPercentage  $$

where:

- C $\in$ { "0 days", "days < 5", "5 < days <= 30", ... } - *Category*;
- S $\in$ { "Green_Concordant", "Green_Discordant", "Yellow_Concordant", ...} - *Strata*:
...
DT_cor <- DT_cor[tolower(pregnancy_start_date_correct) == "y", pregnancy_start_date_difference :="1. 0 days"]
DT_cor <- DT_cor[pregnancy_start_date_difference == "1"| pregnancy_start_date_difference == "5", pregnancy_start_date_difference := "2. days < 5 "]
DT_cor <- DT_cor[pregnancy_start_date_difference == "2"| pregnancy_start_date_difference == "6", pregnancy_start_date_difference := "3. 5 < days <= 30"]
DT_cor <- DT_cor[pregnancy_start_date_difference == "3"| pregnancy_start_date_difference == "7", pregnancy_start_date_difference := "4. 30 < days <= 90"]
DT_cor <- DT_cor[pregnancy_start_date_difference == "4"| pregnancy_start_date_difference == "8", pregnancy_start_date_difference := "5. days > 90"]
DT_cor <- DT_cor[tolower(pregnancy_start_date_correct) == "unk_assessable", pregnancy_start_date_difference := "6. UNK ASSESSABLE"]
DT_cor <- DT_cor[tolower(pregnancy_start_date_correct) == "unk_not_assessable", pregnancy_start_date_difference := "7. UNK NOT ASSESSABLE"]
DT_cor <- DT_cor[tolower(pregnancy_start_date_correct) == "unk_appraisable", pregnancy_start_date_difference := "8. UNK APPRAISABLE"]
DT_cor <- DT_cor[pregnancy_start_date_difference == "" | is.na(pregnancy_start_date_difference), pregnancy_start_date_difference :="9. NOT SPECIFIED"]
...
if(sum(DT_pie$y_pie)<100){
  tmp_pie <- data.table(y_pie = 100 - sum(DT_pie$y_pie),
                    pregnancy_start_date_difference = "10. Not_sampled")
  DT_pie <- rbind(DT_pie, tmp_pie)
}
...
ggplot(df2, aes(x = "" , y = y_pie, fill = fct_inorder(as.factor(pregnancy_start_date_difference)))) +
 ...
 scale_fill_manual(values = c("1. 0 days" = "slategray1", 
                               ...
                               "9. NOT SPECIFIED" = "tan2", 
                              "10. Not_sampled" = "grey")) +
  guides(fill = guide_legend(title = "Start difference")) +
  theme_void()+
  theme(text = element_text(family = "serif"))
...
#### Reproportioning over the color quality.

The following graph shows the expected percentage of pregnancies in each color quality strata with a different start date from that generated by the algorithm. 
The percentages are calculated taking into account the verification results and the sampling fraction.

$$ StartDifference_{C, Q} = \sum_{S \in Q} \frac{N_{C, S}}{N_{S}} \times StrataOverColorPercentage  $$

where:

- C $\in$ { "0 days", "days < 5", "5 < days <= 30", ... } - *Category*;
- S $\in$ { "Green_Concordant", "Green_Discordant", "Yellow_Concordant", ...} - *Strata*:
- Q $\in$ { "Green", "Yellow", "Red", ...} - *Quality color*:
...
The following graph shows the results of the verification for the variable end of pregnancy.

```{r end}
DT_end <- copy(sample_validated)
DT_end <- DT_end[, pregnancy_end_date_difference := as.character(pregnancy_end_date_difference)]

DT_end <- DT_end[tolower(pregnancy_end_date_correct) == "y", pregnancy_end_date_difference :="1. 0 days"]
...
DT_end <- DT_end[tolower(pregnancy_end_date_correct) == "unk_assessable", pregnancy_end_date_difference := "6. UNK ASSESSABLE"]
DT_end <- DT_end[tolower(pregnancy_end_date_correct) == "unk_not_assessable", pregnancy_end_date_difference := "7. UNK NOT ASSESSABLE"]
DT_end <- DT_end[tolower(pregnancy_end_date_correct) == "unk_appraisable", pregnancy_end_date_difference := "8. UNK APPRAISABLE"]
DT_end <- DT_end[pregnancy_end_date_difference == "" | is.na(pregnancy_end_date_difference), pregnancy_end_date_difference :="9. NOT SPECIFIED"]



DT_end[, color := str_split(sample, "_")[[1]][1], sample]

DT_end_agg <- DT_end[, .N, by = c("pregnancy_end_date_difference", "sample")]
DT_end_agg_plt <- merge(DT_end_agg, tmp[, .(strata, N_verified )] , by.x =  "sample", by.y = "strata")


plt <- ggplot(DT_end_agg_plt, aes(x = sample, y =N, fill = pregnancy_end_date_difference)) +
  geom_col()+
 scale_fill_manual(values = c("1. 0 days" = "slategray1", 
                               "2. days < 5 " = "slategray2", 
                               "3. 5 < days <= 30" = "slategray3", 
                               ...
                               "9. NOT SPECIFIED" = "tan2")) +
  scale_x_discrete(guide = guide_axis(angle = 45)) +
  guides(fill = guide_legend(title = "Verification: end difference")) +
  xlab("Strata")+
  ylab("N of pregnancies verified")+
  theme_bw()+
  theme(text = element_text(family = "serif"))
...
DT_cor <- DT_cor[tolower(pregnancy_end_date_correct) == "y", pregnancy_end_date_difference :="1. 0 days"]
DT_cor <- DT_cor[pregnancy_end_date_difference == "1"| pregnancy_end_date_difference == "5", pregnancy_end_date_difference := "2. days < 5 "]
DT_cor <- DT_cor[pregnancy_end_date_difference == "2"| pregnancy_end_date_difference == "6", pregnancy_end_date_difference := "3. 5 < days <= 30"]
DT_cor <- DT_cor[pregnancy_end_date_difference == "3"| pregnancy_end_date_difference == "7", pregnancy_end_date_difference := "4. 30 < days <= 90"]
DT_cor <- DT_cor[pregnancy_end_date_difference == "4"| pregnancy_end_date_difference == "8", pregnancy_end_date_difference := "5. days > 90"]
DT_cor <- DT_cor[tolower(pregnancy_end_date_correct) == "unk_assessable", pregnancy_end_date_difference := "6. UNK ASSESSABLE"]
DT_cor <- DT_cor[tolower(pregnancy_end_date_correct) == "unk_not_assessable", pregnancy_end_date_difference := "7. UNK NOT ASSESSABLE"]
DT_cor <- DT_cor[tolower(pregnancy_end_date_correct) == "unk_appraisable", pregnancy_end_date_difference := "8. UNK APPRAISABLE"]
DT_cor <- DT_cor[pregnancy_end_date_difference == "" | is.na(pregnancy_end_date_difference), pregnancy_end_date_difference :="9. NOT SPECIFIED"]
...
if(sum(DT_pie$y_pie)<100){
  tmp_pie <- data.table(y_pie = 100 - sum(DT_pie$y_pie),
                    pregnancy_end_date_difference = "10. Not_sampled")
  DT_pie <- rbind(DT_pie, tmp_pie)
}
...
The following graph shows the results of the verification for the variable "type of pregnancy end".

```{r type}
DT_type <- copy(sample_validated)
#DT_type <- DT_type[sample == "Yellow_SlightlyDiscordant", sample := "Yellow_CP_SlightlyDiscordant"]
DT_type <- DT_type[type_of_pregnancy_end_correct == "" | 
...
DT_type <- DT_type[tolower(type_of_pregnancy_end_correct) == "no" |
                   tolower(type_of_pregnancy_end_correct) == "n" , 
                 type_of_pregnancy_end_correct :="Incorrect"]

DT_type <- DT_type[type_of_pregnancy_end_correct != "Incorrect" &
                   type_of_pregnancy_end_correct != "Correct" , 
                 type_of_pregnancy_end_correct :="Not known"]

DT_type[, color := str_split(sample, "_")[[1]][1], sample]

DT_type_agg <- DT_type[, .N, by = c("type_of_pregnancy_end_correct", "sample")]
DT_type_agg_plt <- merge(DT_type_agg, tmp[, .(strata, N_verified )] , by.x =  "sample", by.y = "strata")


plt <- ggplot(DT_type_agg_plt, aes(x = sample, y =N, fill = type_of_pregnancy_end_correct)) +
  geom_col()+
    scale_fill_manual(values = c("Correct" = "slategray1", 
                               "Incorrect" = "firebrick", 
                               "Not known" = "gray")) +
  scale_x_discrete(guide = guide_axis(angle = 45)) +
  guides(fill = guide_legend(title = "Verification: type difference")) +
  xlab("Strata")+
  ylab("N of pregnancies verified")+
  theme_bw()+
  theme(text = element_text(family = "serif"))
...
if(sum(DT_pie$y_pie)<100){
  tmp_pie <- data.table(y_pie = 100 - sum(DT_pie$y_pie),
                    type_of_pregnancy_end_correct = "Not_sampled")
  DT_pie <- rbind(DT_pie, tmp_pie)
}
...
### Records of multiple Pregnancies

The following graph shows the results of the verification for the variable "record belonging to multiple pregnancy".

```{r bel}
DT_belong <- copy(sample_validated)
#DT_belong <- DT_belong[sample == "Yellow_SlightlyDiscordant", sample := "Yellow_CP_SlightlyDiscordant"]
DT_belong <- DT_belong[records_belong_to_multiple_pregnancy == "" | 
...
DT_belong <- DT_belong[tolower(records_belong_to_multiple_pregnancy) == "yes" |
                   tolower(records_belong_to_multiple_pregnancy) == "y" , 
                 records_belong_to_multiple_pregnancy :="Yes"]

DT_belong <- DT_belong[records_belong_to_multiple_pregnancy != "No" &
                   records_belong_to_multiple_pregnancy != "Yes" , 
                 records_belong_to_multiple_pregnancy :="Not known"]

DT_belong[, color := str_split(sample, "_")[[1]][1], sample]

DT_belong_agg <- DT_belong[, .N, by = c("records_belong_to_multiple_pregnancy", "sample")]
DT_belong_agg_plt <- merge(DT_belong_agg, tmp[, .(strata, N_verified )] , by.x =  "sample", by.y = "strata")


plt <- ggplot(DT_belong_agg_plt, aes(x = sample, y =N, fill = records_belong_to_multiple_pregnancy)) +
  geom_col()+
    scale_fill_manual(values = c("No" = "slategray1", 
                               "Yes" = "firebrick", 
                               "Not known" = "gray")) +
  scale_x_discrete(guide = guide_axis(angle = 45)) +
  guides(fill = guide_legend(title = "Verification: records of multiple pregnancy")) +
  xlab("Strata")+
  ylab("N of pregnancies verified")+
  theme_bw()+
  theme(text = element_text(family = "serif"))
...
if(sum(DT_pie$y_pie)<100){
  tmp_pie <- data.table(y_pie = 100 - sum(DT_pie$y_pie),
                    records_belong_to_multiple_pregnancy = "Not_sampled")
  DT_pie <- rbind(DT_pie, tmp_pie)
}
