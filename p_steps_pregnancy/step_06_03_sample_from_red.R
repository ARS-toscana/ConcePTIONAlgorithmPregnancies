...
### 7: sample from red 
l <- D3_pregnancy_reconciled_valid[highest_quality == "4_red", .N]
if (l > 0){
  sample_id_red <- sample(x = D3_pregnancy_reconciled_valid[highest_quality == "4_red", pregnancy_id], size =  min(l, red_sample_size), replace = FALSE)
  list_of_sample[["sample_red_record"]] <- D3_pregnancy_reconciled_valid[pregnancy_id %in% sample_id_red][,sample:="red_quality"]
}
...
fwrite(sample_from_pregnancies_anon, paste0(dirvalidation, "/sample_from_pregnancies_anon_red_quality_", now, ".csv"))
