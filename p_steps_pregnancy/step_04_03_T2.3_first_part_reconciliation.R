...
gop_ybr_Ingreen<-unique(gop_ybr[gop_green,.(pregnancy_id,person_id,record_date=x.record_date,pregnancy_start_date,meaning_start_date,pregnancy_ongoing_date,meaning_ongoing_date,pregnancy_end_date,meaning_end_date,type_of_pregnancy_end,codvar, coding_system,imputed_start_of_pregnancy,imputed_end_of_pregnancy,meaning,survey_id,visit_occurrence_id,PROMPT,EUROCAT,CONCEPTSETS,CONCEPTSET,ITEMSETS,coloured_order,order_quality,ID,group_identifier, origin),on =.(person_id==person_id, record_date<=group_end_date_28, record_date>=group_start_date_28), nomatch=0L])
...
suppressWarnings(gop_gybr1<-gop_gybr1[,overlap:=max(overlap),by=.(person_id, group_identifier)])
## added 08/10
suppressWarnings(gop_gybr1<-gop_gybr1[,overlapMAX:=max(overlap),by=.(person_id)])
...
suppressWarnings(gop_gybr3<-gop_gybr3[,overlap:=max(overlap),by=.(person_id, group_identifier)])
## added 08/10
suppressWarnings(gop_gybr3<-gop_gybr3[,overlapMAX:=max(overlap),by=.(person_id)])
...
suppressWarnings(gop_gybr4<-gop_gybr4[,overlap:=max(overlap),by=.(person_id, group_identifier)])
## added 08/10
suppressWarnings(gop_gybr4<-gop_gybr4[,overlapMAX:=max(overlap),by=.(person_id)])
...
if(HTML_files_creation){
  cat("Describing D3_groups_of_pregnancies \n")
