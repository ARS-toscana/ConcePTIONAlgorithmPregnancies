if (this_datasource_has_itemsets_stream_from_medical_obs){
  ...
  study_itemset_of_pregnancy <- c("LastMestrualPeriod",
                                  "GestationalAge", 
                                  "PregnancyTest", 
  ...
  for (itemvar in study_itemset_of_pregnancy){
    ...
    dataset_item_sets <- rbind(dataset_item_sets,
  ...
  if (thisdatasource=="VID"){
    ...
    dataset_item_sets<-dataset_item_sets[,`:=`(pregnancy_ongoing_date=date, 
                                               meaning_ongoing_date=paste0("from_itemset_",item_set))]
    
    # then pregnancy_start_date
    dataset_item_sets<-dataset_item_sets[item_set=="GestationalAge" & 
                                           mo_unit=="weeks", 
                                         `:=`(pregnancy_start_date=pregnancy_ongoing_date-(as.numeric(mo_source_value)*7),  
    ...
    dataset_item_sets<-dataset_item_sets[, `:=`(pregnancy_end_date = pregnancy_start_date + 280, 
  ...
  if (thisdatasource=="BIFAP"){
    ...
    dataset_item_sets<-dataset_item_sets[item_set=="GestationalAge" & mo_unit=="days", 
                                         `:=`(pregnancy_start_date=pregnancy_ongoing_date-mo_source_value, 
                                              meaning_start_date=paste0("from_itemset_",item_set)) ]
    
    dataset_item_sets<-dataset_item_sets[item_set=="GestationalAge" & mo_unit=="weeks", 
                                         `:=`(pregnancy_start_date=pregnancy_ongoing_date-(mo_source_value*7), 
                                              meaning_start_date=paste0("from_itemset_",item_set)) ]
      
    dataset_item_sets<-dataset_item_sets[item_set=="LastMestrualPeriod", 
                                         `:=`(pregnancy_start_date=ymd(mo_source_value), 
                                              meaning_start_date=paste0("from_itemset_",item_set)) ]
  ...
  if (thisdatasource=="EpiChron"){
    ...
    # then pregnancy_start_date
    dataset_item_sets<-dataset_item_sets[item_set=="LastMestrualPeriodImplyingPregnancy", 
                                         `:=`(pregnancy_start_date=ymd(mo_source_value), 
                                              meaning_start_date=paste0("from_itemset_",item_set)) ]
  ...
  if ( thisdatasource=="HSD" | thisdatasource=="PHARMO" | thisdatasource== "CASERTA"){
    ...
    dataset_item_sets<-dataset_item_sets[item_set=="PregnancyTest" & 
                                           mo_source_value==unlist(dictonary_of_itemset_PregnancyTest_thisdatasource$PregnancyTest),]
                                         
    # start creating pregnancy_ongoing_date
    dataset_item_sets<-dataset_item_sets[,`:=`(pregnancy_ongoing_date=date, 
                                               meaning_ongoing_date=paste0("from_itemset_",item_set,"_positive"))]
    
    # then pregnancy_start_date
    dataset_item_sets<-dataset_item_sets[item_set=="PregnancyTest", 
                                         `:=`(pregnancy_start_date=pregnancy_ongoing_date - days_from_start_PregnancyTest_thisdatasource, 
                                              meaning_start_date=paste0("from_itemset_",item_set,"_positive")) ]

    
    #the pregnancy is ongoing and has a start date but has no end, then at term end of the pregnancy is assumed for the imputation
    dataset_item_sets<-dataset_item_sets[item_set=="PregnancyTest", 
                                         `:=`(pregnancy_end_date = pregnancy_start_date + days_to_end_PregnancyTest_thisdatasource, 
                                              imputed_end_of_pregnancy=1,  
                                              imputed_start_of_pregnancy=0, 
                                              meaning_end_date=paste0("imputed_itemset_from_",item_set,"_positive"))]
  ...
  D3_Stream_ITEMSETS <- dataset_item_sets[,.(pregnancy_id,
                                             person_id,
                                             record_date,
